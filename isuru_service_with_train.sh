...
for TestName in ${TestNameArray[*]}; do
  ...
  for ConcurrencyPlan in ${ConcurrencyPlanArray[*]}; do
    ...
    docker run --publish 5000:5000 --name tuner_service -d --net myNet --ip 192.168.1.3 --cpus=1 nayanagamuhandiram/python_service:hpc_testing_5  python app.py "$TestName"/"$ConcurrencyPlan"
    docker run --publish 15000:15000 --name java_service -d --net myNet --ip 192.168.1.4 --volume /home/nayananga/output/isuru/java_service/with_train/"$TestName"/"$ConcurrencyPlan":/home/log --cpus=1 nayanagamuhandiram/java_service java -jar adaptive-concurrency-control-1.0-SNAPSHOT-jar-with-dependencies.jar "$TestName" 10 99P
